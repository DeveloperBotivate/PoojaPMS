...
import { Clock, History as HistoryIcon, Check, FileUp, Eye, Search, RotateCcw } from 'lucide-react';
import { getProjects, getDrawingUploads, isDrawingUploadComplete, getDesignByProject } from '../../utils/storageManager';
...
export default function UploadDesignPage() {
  ...
  const [drawingUploads, setDrawingUploads] = useState([]);
  ...
  const refresh = () => {
    // Only projects that have completed the Drawing Upload stage (sitting in that page's
    // History tab) are eligible for Upload Design.
    const uploads = getDrawingUploads();
    const eligibleProjects = getProjects().filter(p => {
      const rec = uploads.find(u => u.projectNo === p.serialNo);
      return isDrawingUploadComplete(rec);
    });
    setProjects(eligibleProjects);
    setDrawingUploads(uploads);
    const counts = {};
    eligibleProjects.forEach(p => {
      counts[p.serialNo] = getDesignByProject(p.serialNo).length;
    });
  ...
  const designCountFor = (projectNo) => designCounts[projectNo] || 0;
  const drawingUploadFor = (projectNo) => drawingUploads.find(u => u.projectNo === projectNo) || null;

  const pendingProjects = projects.filter(p => designCountFor(p.serialNo) === 0).reverse();
  const historyProjects = projects.filter(p => designCountFor(p.serialNo) > 0).reverse();
  ...
  const tableHeaders = [
    "Project No", "Project Type", "Project Name", "Contractor Name", "Contractor Phone No",
    ...
    "Work Period", "Maintenance Period", "Drawing Upload", "Design Status"
  ];

  const renderRow = (item) => {
    const count = designCountFor(item.serialNo);
    const drawing = drawingUploadFor(item.serialNo);
    return (
      <tr
        key={item.serialNo}
        onClick={() => navigate(`/project-minors/${encodeURIComponent(item.serialNo)}`)}
        className="hover:bg-indigo-50/30 transition-colors border-b border-gray-100 cursor-pointer"
      >
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
          {drawing ? (
            <div>
              <div className="text-[10px] text-gray-500">{formatDate(drawing.drawingDate)}</div>
              {drawing.drawingFile && (
                <a
                  href={drawing.drawingFile}
                  target="_blank"
                  rel="noreferrer"
                  onClick={(e) => e.stopPropagation()}
                  className="inline-flex items-center gap-1 text-[10px] font-semibold text-indigo-600 hover:underline truncate max-w-[140px]"
                  title={drawing.drawingFileName}
                >
                  <Eye size={11} /> {drawing.drawingFileName || 'View file'}
                </a>
              )}
            </div>
          ) : (
        ...
        <td className="px-4 py-3 text-center whitespace-nowrap">
          {count > 0 ? (
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded border bg-emerald-100 text-emerald-700 border-emerald-200">
              <Check size={11} /> {count} rows
            </span>
          ) : (
  ...
  const renderCard = (item) => {
    ...
    return (
      <div
        key={item.serialNo}
        onClick={() => navigate(`/project-minors/${encodeURIComponent(item.serialNo)}`)}
        className="bg-white rounded-lg border border-indigo-50 shadow-sm p-3 space-y-2 cursor-pointer active:bg-indigo-50/40 transition-colors"
      >
        <div className="flex justify-between items-start border-b border-gray-100 pb-2">
          ...
          {count > 0 ? (
            <span className="inline-flex items-center gap-1 text-[9px] font-semibold uppercase px-1.5 py-0.5 rounded border bg-emerald-100 text-emerald-700 border-emerald-200 flex-shrink-0">
              <Check size={10} /> {count} rows
            </span>
          ) : (
            <span className="inline-block text-[9px] font-semibold uppercase px-1.5 py-0.5 rounded border bg-gray-100 text-gray-500 border-gray-200 flex-shrink-0">Pending</span>
          )}
        </div>

        <div className="bg-slate-50 rounded p-2 text-[10px] space-y-1">
          <div className="flex justify-between items-center" onClick={(e) => e.stopPropagation()}>
            <span className="text-gray-500">Drawing Upload:</span>
            {drawing ? (
              <a
                href={drawing.drawingFile}
                target="_blank"
              ...
              >
                <Eye size={11} /> {formatDate(drawing.drawingDate)}
              </a>
            ) : (
